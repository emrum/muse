...
void View::mousePressEvent(QMouseEvent* ev)
      {
      QMouseEvent e(ev->type(), mapDev(ev->position().toPoint()),
...
void View::dropEvent(QDropEvent* ev)
      ...
      QDropEvent nev(mapDev(ev->position().toPoint()), ev->possibleActions(), ev->mimeData(), ev->buttons(), ev->modifiers(), ev->type());     
      //viewDropEvent(ev);
      viewDropEvent(&nev);
      }
