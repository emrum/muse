...
void TList::mouseDoubleClickEvent(QMouseEvent* ev)
{
    ...
    MusECore::Track* t = y2Track(qRound(ev->position().y()) + ypos);
    if(t == nullptr)
    {
        ev->accept();
        return;
...
void TList::mousePressEvent(QMouseEvent* ev)
{
    ...
    const int x       = qRound(ev->position().x());
    const int y       = qRound(ev->position().y());
